...
func (a *applicationCreator) Add(authToken string, token string) error {
	logger.Debugf("add token %s to user with token %s", token, authToken)
	start := time.Now()
	defer logger.Debugf("add token completed in %dms", time.Now().Sub(start)/time.Millisecond)
	...
	content, err := json.Marshal(&api.AddTokenRequest{
	...
	logger.Debugf("send add token request to auth api: %s", string(content))
	...
	if resp.StatusCode/100 != 2 {
		return fmt.Errorf("add token request to auth api failed with status: %d", resp.StatusCode)
	}
	return nil
}
