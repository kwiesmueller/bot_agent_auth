...
func TestMatchTrue(t *testing.T) {
	...
	match := c.Match(&api.Request{
		Message: "/auth add token token123 from user user123",
	})
...
func TestMatchFalse(t *testing.T) {
	c := New("/auth", nil)
	match := c.Match(&api.Request{
		Message: "/auth add token token123 from user",
	})
...
func TestHandleMessageSuccess(t *testing.T) {
	...
	responses, err := c.HandleMessage(&api.Request{
		Message:   fmt.Sprintf("/auth add token %v from user %v", token, username),
		AuthToken: authToken,
	})
